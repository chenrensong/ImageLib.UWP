...
static int PaletteCompareColorsForQsort(const void* p1, const void* p2) {
  const uint32_t a =(uint32_t) WebPMemToUint32((uint8_t*)p1);
  const uint32_t b = (uint32_t)WebPMemToUint32((uint8_t*)p2);
  assert(a != b);
  return (a < b) ? -1 : 1;
}
  ...
  if (histo != NULL) {
    ...
    }
    {
      ...
      *min_entropy_ix = kDirect;
      for (k =(EntropyIx)( kDirect + 1); k <= last_mode_to_analyze; k=(EntropyIx) (k+1)) {
